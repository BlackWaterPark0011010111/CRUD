...
Также идемпотентность через бизнес ключ,семантическая идемпотентность. через CREATE при создании нового юзера,не через логику чтобы он существовал с этой секунды,а как обеспечение того, чтобы такой юзер существовал с такими данными "Вообще". при регистрации нового юзера ,и если такой емейл уже есть в базе,то выводится код ошибки или конфликта и сам статус в зависимости от семантики.и операция будет идемпотентна в своей манере и природе.потому что повторный вызов с этим емейл не будет менять состояние системы.
READ(GET) - он сам идемпотентен по определению. он не меняет свое состояние и при повторном чтении,будет давать свой результат,тот же ,что если бы данные не менялись в своем промежутке. но имеет побочные эффекты при записи логов в аналитику. и такой GET перестает быть идемпотентным,потому что чистая идемпотентность READ- это договоренность что операция только наблюдает а не мутирует.
...
PATCH - будет частичным обновлением. его семантика это "примени к ресурсу входные изменения!".TODO:НО,если изменения выражаются в виде дельта-операций(инкремент,добавление в массив или перемещение),то PATCH он будет не идемпотентным. 
Мы можем увеличить счетчик например. но если изменения будут в качестве новых значений для полей TODO: в? или по типу? JSON Merge Patch или JSON Patch операция replace, то такая операция,она может быть идемпотентной.
